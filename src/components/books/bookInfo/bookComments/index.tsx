...
const BookComments: React.FC<{ bookId: string }> = ({ bookId }) => {
  ...
  const removeBookOpinionLike = useCallback(
    (userId: string) => {
  ...
  const addBookOpinionLike = useCallback(
    (userId: string, hasDislike: boolean) => {
  ...
  const removeBookOpinionDislike = useCallback(
    (userId: string) => {
  ...
  const addBookOpinionDislike = useCallback(
    (userId: string, hasLike: boolean) => {
      checkIsAuthenticated();
  ...
  const data = bookOpinions.map((o) => ({
    ...
    avatar: (
      <Link to={`${ROUT_PAGE_NAME.USER_PROFILE}?userId=${o.user.id}`}>
        <Image
          preview={false}
          style={{ width: '40px', maxHeight: '40px', borderRadius: '10px' }}
          src={'data:image/png;base64,' + o.user.avatarDataBase64}
        />
      </Link>
    ),
    ...
    likes: o.hasLike ? (
      <Button onClick={() => removeBookOpinionLike(o.user.id)}>
        <LikeTwoTone /> {o.likesCount}
      </Button>
    ) : (
      <Button onClick={() => addBookOpinionLike(o.user.id, o.hasDislike)}>
        <LikeOutlined /> {o.likesCount}
      </Button>
    ),
    dislikes: o.hasDislike ? (
      <Button onClick={() => removeBookOpinionDislike(o.user.id)}>
        <DislikeTwoTone /> {o.dislikesCount}
      </Button>
    ) : (
      <Button onClick={() => addBookOpinionDislike(o.user.id, o.hasLike)}>
        <DislikeOutlined /> {o.dislikesCount}
      </Button>
    ),
  }));
