...
const ReadingCalendarStatistic: React.FC<{ statistic: ActiveBooksStatistic; userId?: string }> = ({
  statistic,
  userId,
}) => {
  ...
  const createCalendarStatistics = () => {
    ...
    for (let i = 0; i < rowCount; i++) {
      result.push(
        <tr key={i} style={{ height: '10px' }}>
          {createRowStatistics(new Date(currentDate))}
        </tr>,
      );
      ...
      rowStatistics.unshift(
        <Tooltip key={formatDate} title={tooltipTitle}>
          <td
            ...
            tabIndex={-1}
            onClick={() => onClickDayStatistics(formatDateForDayStatistic)}
            className="calendarReading-day"
  ...
  return (
    <>
      <Col style={{ marginTop: '100px', textAlign: 'center', overflowX: 'auto' }}>
        <div style={{ display: 'inline-block' }}>
          <div style={{ textAlign: 'left', paddingBottom: '10px' }}>
            {statistic.numberPagesReadPerYear} pages read per year
          </div>
          <table id="calendarStatistics-table">
            ...
            <tbody>{createCalendarStatistics()}</tbody>
          </table>
        </div>
      </Col>
      ...
      <StatisticsPerDay
        day={curDaySelected}
        show={showStatisticsPerDay}
        userId={userId}
        onClose={() => {
          setShowStatisticsPerDay(false);
        }}
      />
    </>
  );
