        ...
        message: (
          <Avatar>
            <Image
              ...
              preview={false}
              width={'32px'}
              src={'data:image/png;base64,' + data.FromUserAvatarDataBase64}
              onClick={() => navigate(`/profile?userId=${data.FromUserId}`)}
            />
          </Avatar>
        ),
        description: data.MessageNotification,
      ...
      <Row style={{ padding: '20px', borderBottom: '1px solid #d0d7de', width: '100%' }} key={n.id}>
        <Col style={{ width: '85%' }}>
          <Link
            ...
            to={`/profile?userId=${n.fromUser?.id}`}
          >
            <Avatar>
              <Image
                preview={false}
                width={'32px'}
                src={'data:image/png;base64,' + n.fromUser?.avatarDataBase64}
              />
            </Avatar>
          </Link>
        </Col>
        <Col style={{ width: '85%' }}>{n.description}</Col>
  ...
  return (
    <Col style={{ float: 'right' }}>
      <Col>
        <Col
          style={{ float: 'right' }}
          onMouseEnter={() => changeDisplayNotificationsList(!isOpenNotificationsList)}
        >
          <Badge
            color="red"
      ...
      <Col
        onMouseLeave={() => changeDisplayNotificationsList(!isOpenNotificationsList)}
        style={{
          width: '450px',
